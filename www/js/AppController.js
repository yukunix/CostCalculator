function HomeControllar($scope, $rootScope, $location, $timeout) {
	...
	$scope.calculateNetAmount = function() {
		...
		total+=$scope.convertBasedOnFrequency5($scope.amount4.amount, $scope.amount4.selected);;
	...
	$scope.netAmountCalculator = function() {
		return parseInt(($scope.calculateNetAmount()-($scope.callculationObject.nagativeAmout*$scope.callculationObject.nagativeOptionAmount))/$scope.callculationObject.netOptionAmount);
	};
